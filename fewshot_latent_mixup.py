...
from v2vNet import Vox2Vox_Mixup, VoxEncoder
...
class V2VDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            gt_voxel_F = lam * gt_voxel + (1 - lam) * mixup_voxel
        ...
        return rendering_image, template_voxel, mixup_image, mixup_template_voxel, gt_voxel_F, lam, negative_voxel, gt_voxel, mixup_voxel
...
#Train from best
checkpoint = torch.load('put_input_mixup_ckpt_here')

#Train from base

v2vnet = Vox2Vox_Mixup()
...
v2v_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(v2v_optimizer, mode='min', patience=10)

encoder_optimizer = torch.optim.Adam(gt_encoder.parameters(), lr = ENCODER_LEARNING_RATE)
encoder_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(encoder_optimizer, mode='min', patience=10)

def FocalLoss(pred, target, lam):
    eps = 1e-6
    alpha = 0.8
...
w_triplet = 0.
best_loss = 1000000
saving_dir = 'ckpts_fewshot/lolshot_mix_latent'+datetime.now().strftime('%Y-%m-%d_%H-%M-%S')+'/'
# model_dir = 'ckpts/MixupVis'+datetime.now().strftime('%Y-%m-%d_%H-%M-%S')+'/'
os.makedirs(saving_dir)
# os.makedirs(model_dir)


IOU_thres = [0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.7]
...
for epoch in range(NUM_EPOCHES):
    ...
    for idx, (images_1, template_voxels_1, images_2, template_voxels_2, gt_voxels, lam, negative_voxels, gtv_1, gtv_2) in enumerate(tqdm(train_dataloader)):
        images_1 = images_1.cuda()
        template_voxels_1 = template_voxels_1.cuda()
        images_2 = images_2.cuda()
        template_voxels_2 = template_voxels_2.cuda()
        ...
        encoded1, encoded2, mixed_encoded, out = v2vnet(images_1, template_voxels_1, images_2, template_voxels_2, lam)
        gtEncoded = gt_encoder(gt_voxels).view(-1, 256*5*5*5)
        gt1Encoded = gt_encoder(gtv_1).view(-1, 256*5*5*5)
        gt2Encoded = gt_encoder(gtv_2).view(-1, 256*5*5*5)
        mixed_encoded = mixed_encoded.view(-1, 256*5*5*5)
        encoded1 = encoded1.view(-1, 256*5*5*5)
        encoded2 = encoded2.view(-1, 256*5*5*5)
        negEncoded = gt_encoder(negative_voxels).view(-1, 256*5*5*5)

        # focal_loss = FocalLoss(out, gt_voxels, lam)
        bce_loss = nn.BCELoss()(out, gt_voxels)
        cos_loss = cosSimLoss(mixed_encoded, gtEncoded)
        # triplet_loss1 = tripletLoss(encoded1, gt1Encoded, negEncoded)
        # triplet_loss2 = tripletLoss(encoded2, gt2Encoded, negEncoded)
        # triplet_loss3 = tripletLoss(mixed_encoded, gtEncoded, negEncoded)
        # triplet_loss = (triplet_loss1 + triplet_loss2 + triplet_loss3)/3
        triplet_loss = tripletLoss(mixed_encoded, gtEncoded, negEncoded)
        ...
        if idx % 10 == 0:
            print("Epoch {} Iteration {}, Current Loss: {}, FL: {}, tp: {}, cos: {}".format(epoch, idx, loss, bce_loss, triplet_loss, cos_loss))
    v2vnet.eval()
    gt_encoder.eval()
    ...
    with torch.no_grad():
        ...
        loss_total = 0
        iou_total = 0
        for idx, (images, template_voxels,_,_, gt_voxels,lam, _, _, _) in enumerate(tqdm(test_dataloader)):
            images = images.cuda()
            template_voxels = template_voxels.cuda()
            lam = lam.cuda()
            gt_voxels = gt_voxels.cuda()
            _, _, encoded, out = v2vnet(imgs_1=images, voxels_1=template_voxels)
            gtEncoded = gt_encoder(gt_voxels).view(-1, 256*5*5*5)
